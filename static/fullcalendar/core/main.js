...
(function (global, factory) {
    ...
    */
    ...
    function parseDateSpanDayGrid(raw, dateEnv, defaultDuration) {
        var span = parseOpenDateSpan(raw, dateEnv);
        var range = span.range;
        if (!range.start) {
            return null;
        ...
        // ugly workaround, because we always substract 1 day in select callback
        if (range.end){
            range.end = dateEnv.add(range.end, defaultDuration);
        }
        return span;
    }
    /*
    ...
    */
    ...
    var Calendar = /** @class */ (function () {
        ...
        */
        ...
        Calendar.prototype.selectDayGrid = function (dateOrObj, endDate, just_trigger_click=false, pev=null) {
            var selectionInput;
            if (endDate == null) {
                if (dateOrObj.start != null) {
                    selectionInput = dateOrObj;
            ...
            // detect if click or select is triggered
            if (just_trigger_click){
                var selection = parseDateSpan(selectionInput, this.dateEnv, createDuration({ days: 1 })); // TODO: cache this?
                if (selection) { // throw parse error otherwise?
                    this.dispatch({ type: 'SELECT_DATES', selection: selection });
                    this.triggerDateClick(selection, pev.target, this.view, pev) ;
                }
            }
            else{
                var selection = parseDateSpanDayGrid(selectionInput, this.dateEnv, createDuration({ days: 1 }) // TODO: cache this?
                );

                if (selection) { // throw parse error otherwise?
                    this.dispatch({ type: 'SELECT_DATES', selection: selection });
                    this.triggerDateSelectDayGrid(selection, pev);
                }
            }
        };
        ...
        // ugly workaround, just use pev, dont think it can be undefined
        Calendar.prototype.triggerDateSelectDayGrid = function (selection, pev) {
            var arg = this.buildDateSpanApi(selection);
            arg.jsEvent = pev;
            arg.view = this.view;
            this.publiclyTrigger('select', [arg]);
